...
public class ApplyController {
	...
	///////////장바구니 기간 넘겨주기///////////
	public String likeMain(@RequestParam String userid ,String start, String end, Model model) {

		String year = getYear();
		String semester = getSemester();

		//System.out.println("likeMain start------------------------------------?"+start);
		//System.out.println("likeMain userid------------------------------------?"+userid);
		model.addAttribute("userid", userid);
	...
	@GetMapping(value = "likeDelete")
	public String likeDelete(Long lecId, String userid, Model model,HttpServletRequest request){
		Long gubun = 1L;
		as.delete(lecId,userid,gubun);
		model.addAttribute("userid",userid);
		model.addAttribute("lecName","");
	...
	public void fileDownload(HttpServletResponse response, Long lecId) throws IOException {
		Optional<Lecture> lecture = as.findByLecId(lecId);
		String filePath = lecture.get().getFilePath();
		System.out.println("파일경롯ㅂ=----------------------------------------------"+filePath);
		try {
	...
	public String planInsert(HttpServletRequest request, @RequestPart MultipartFile file1, Long lecId, String referer, Model model) throws Exception {
		System.out.println("id넘어오나-----------------"+lecId);

		Optional<Lecture> lecture = as.findByLecId(lecId);
		//Lecture lecture = as.findByLecId(lecId);
		//Report report = new Report();
		//			ApplicationLec applicationLec = as.findByIdName(userid, lecId);
		String fileRealName = file1.getOriginalFilename(); //파일명
		...
		//상대 경로 지정
		String path = request.getSession().getServletContext().getRealPath("/upload/plan");
		/*System.out.println("--------------------과제 upload path---------------------------"+path);*/
		File savePath = new File(path); //realPath 경로
		if (!savePath.exists()) { //realPath 경로에 파일 업로드 하기 위한 폴더가 없으면 생성
			savePath.mkdirs();
		...
		try {
			file1.transferTo(saveFile); //파일 저장
			lecture.get().setFilePath(path);
			lecture.get().setFileName(fileRealName);
			as.savePlan(lecture.get());
			//lecture.setFileName(fileRealName);
			//as.savePlan(lecture);
